...
namespace syn
{
    namespace Impl
    {
        ...
        struct Snapshot
        {
            ...
            inline static auto getJsonDiff(
                const ssvj::Val& mX, const ssvj::Val& mY)
            {
            ...
            inline auto getDiffWith(const Snapshot& mX)
            {
                ...
                ssvu::tplForData(
                    [this, &result, &mX](auto mD, auto& mTDCurrent,
                        auto& mTDOther, auto& mTDDiff) mutable
                    {
                        ...
                        for(auto i(0u); i < maxObjs; ++i)
                        {
                            ...
                            if(diffBitsetToRemove[i])
                                mTDDiff.toRemove.emplace_back(i);
                            if(diffBitsetToUpdate[i])
                            {
                                auto objDiff(Snapshot<TManager>::getJsonDiff(
                                    mTDCurrent.items.at(i),
                                    mTDOther.items.at(i)));
                                ...
                                if(!objDiff.isEmptyObj())
                                    mTDDiff.toUpdate[i] = objDiff;
                            }
                        }
                    },
