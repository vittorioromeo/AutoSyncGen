...
namespace Impl
{
    template <typename TSettings, typename TSPT, typename TRPT,
    ...
    private:
        ...
        {
            return ssvu::castUp<Derived>(*this);
        }
        ...
        inline void tryBindSocket()
        {
            ...
            {
                debugLo() << "Socket successfully bound to port " +
                             ssvu::toStr(port) + "\n";
            }
        }
        ...
        inline void receiveThread()
        {
            while(true) {
                ...
                if(socket.receive(recvBuffer, senderIp, senderPort) !=
                   sf::Socket::Done)
                {
    ...
    protected:
        ...
        inline void mkPacket(TArgs&&... mArgs)
        {
            sendBuffer.clear();
    ...
    public:
        inline SessionHost(std::string mName, syn::Port mPort)
            : name{mName}, ip{IpAddress::getLocalAddress()}, port{mPort}
        {
            socket.setBlocking(true);
            tryBindSocket();
            hostFuture = std::async(std::launch::async, [this]
            {
                receiveThread();
            });
        }
    ...
    using SessionServerBase =
    SessionHost<TSettings, PT::StoC, PT::CtoS, SessionServer>;
    template <typename TSettings>
    using SessionClientBase =
    SessionHost<TSettings, PT::CtoS, PT::StoC, SessionClient>;
}
}
