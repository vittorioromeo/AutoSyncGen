...
namespace syn
{
    namespace PT
    {
        ...
        {
            ConnectionAccept = 0,  // Accept a requested client connection,
                                   // assigning the client a CID
            ConnectionDecline = 1, // Decline a client's requested connection

            SyncRequestSatisfy =
                2, // Satisfy sync request (if the client's revision is behind)
            SyncRequestUnneeded = 3, // Sync not needed (client has the same
                                     // revision as the server)
            SyncRequestDecline =
