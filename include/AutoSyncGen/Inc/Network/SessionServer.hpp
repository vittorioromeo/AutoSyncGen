...
class SessionServer : public Impl::SessionServerBase<TSettings>
{
...
private:
    ...
    inline void sendToClient(CID mCID, TArgs&&... mArgs)
    {
        if(!cManager.has(mCID)) {
            this->debugLo() << "Wanted to send a packet to CID " << mCID
    ...
    inline void handle(RPT mType)
    {
        ...
        if(!cManager.has(cid)) {
            this->debugLo() << "Received a packet from non-connected CID "
                            << cid << "\n";
            return;
        }
        ...
        switch(mType)
        {
            case RPT::ConnectionRequest:
                ...
                "This packet has to be handle in a special way");
            case RPT::Ping: handlePing(cid); return;
            case RPT::SyncRequest: handleSyncRequest(cid); return;
            case RPT::Data: this->onDataReceived(cid, this->recvBuffer); return;
    ...
    inline void handleConnectionRequest()
    {
        ...
        auto cid(
        cManager.acceptClient(this->senderIp, this->senderPort).getCID());
        sendConnectionAccept(cid);
    }

    inline void handlePing(CID mCID) { cManager.pingReceived(mCID); }
    ...
    {
        sendToClient<SPT::ConnectionAccept>(
        mCID, mCID, "Welcome to the server.");
    }

public:
    ssvu::Delegate<void(CID, sf::Packet&)> onDataReceived;

    inline SessionServer(syn::Port mPort)
        : Impl::SessionServerBase<TSettings>{"Server", mPort}
    {
        this->setBusy(true);
    }

    template <typename... TArgs>
    inline void sendDataToClient(CID mCID, TArgs&&... mArgs)
    {
