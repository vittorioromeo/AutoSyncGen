...
template <>
struct LifetimeManager<Message>
{
    ...
    /// @brief Required function that creates an object and returns an handle to
    /// it.
    inline Handle create() { return &ssvu::getEmplaceUPtr<Message>(storage); }

    /// @brief Required function that removes an object attached to a specific
    /// handle.
...
private:
    ...
    template <typename T>
    inline int getChoice(const std::string& mMsg, const T& mDefault)
    {
        ssvu::lo() << mMsg << "Choose: \n"
                   << "	0. Default (" << mDefault << ")\n"
                   << "	1. Enter manually\n";
